...
def _write_t05_run(path: Path, start_order: str) -> None:
    path.mkdir()
    observation = _t05_observation()
    for device_id, role in enumerate(EndpointRole):
        endpoint = {
            "cleanup": {
                "device_kernel": "CLOSED",
            ...
            "device_loop": {
                ...
                "launches": 1,
                "report": (
                    _t05_driver_report() if role is EndpointRole.ATTENTION else observation.service_report
                ).to_dict(),
            },
            "error": None,
            "manifest": {"opaque_handle": {"kind": "ACL_VMM_SHAREABLE_HANDLE", "sha256": "a" * 64}},
    ...
    result = {
        ...
        "c2_status": "NOT_ESTABLISHED",
        "claim_scope": "NPU_SURROGATE_ONLY",
        "data_results": {"T05": {"passed": 100, "status": "PASS"}},
        "host_bounce_bytes": 0,
        "host_hot_path": {
            "completion_messages": 0,
            "control_messages": 0,
...
def test_collect_validates_t05_runs_and_preserves_claim_boundary(tmp_path):
    first = tmp_path / "first"
    second = tmp_path / "second"
    _write_t05_run(first, "attention-first")
    _write_t05_run(second, "wse-first")
    evidence = collect_stage1b(
        successful_runs=(("task-a", first), ("task-b", second)),
        implementation_revision="abc123",
        case_id="T05",
...
def _write_t07_run(path: Path, start_order: str) -> None:
    path.mkdir()
    observation = _t07_observation()
    for device_id, role in enumerate(EndpointRole):
        endpoint = {
            "cleanup": {
                "device_kernel": "CLOSED",
            ...
            "device_loop": {
                ...
                "launches": 1,
                "report": (
                    _t07_driver_report() if role is EndpointRole.ATTENTION else observation.service_report
                ).to_dict(),
            },
            "error": None,
            "manifest": {"opaque_handle": {"kind": "ACL_VMM_SHAREABLE_HANDLE", "sha256": "a" * 64}},
    ...
    result = {
        ...
        "c2_status": "NOT_ESTABLISHED",
        "claim_scope": "NPU_SURROGATE_ONLY",
        "data_results": {"T07": {"passed": 100, "status": "PASS"}},
        "host_bounce_bytes": 0,
        "host_hot_path": {
            "completion_messages": 0,
            "control_messages": 0,
...
def test_collect_validates_t07_runs_and_preserves_claim_boundary(tmp_path):
    first = tmp_path / "first"
    second = tmp_path / "second"
    _write_t07_run(first, "attention-first")
    _write_t07_run(second, "wse-first")
    evidence = collect_stage1b(
        successful_runs=(("task-a", first), ("task-b", second)),
        implementation_revision="abc123",
        case_id="T07",
