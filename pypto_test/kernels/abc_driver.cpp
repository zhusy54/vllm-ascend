...
static const KernelMeta g_abc_driver_meta
    __attribute__((used, section(".ascend.meta.pypto_abc_driver_0_mix_aiv"))) = {
...
__aicore__ inline void PublishDescriptor(
    __gm__ DescriptorLine *descriptor, uint64_t generation, uint64_t request_id,
...
extern "C" __global__ __aicore__ void pypto_abc_driver_0_mix_aiv(
    __gm__ uint32_t *local_input, __gm__ uint32_t *local_b_output,
    __gm__ uint32_t *local_final_output, __gm__ SignalLine *host_request_signal,
    __gm__ DescriptorLine *host_request_descriptor, __gm__ SignalLine *host_result_signal,
    __gm__ DescriptorLine *host_result_descriptor, __gm__ SignalLine *b_completion_signal,
    __gm__ DescriptorLine *b_completion_descriptor, __gm__ LifecycleLine *lifecycle,
    __gm__ DriverReport *report_address, __gm__ uint32_t *remote_b_input,
    ...
    while (Load64(&lifecycle->stop_requested) == 0) {
        ...
        PublishDescriptor(
            remote_submission_descriptor, generation, request_id, elements, a_checksum, sequence, 0
        );
        dsb(DSB_ALL);
        ...
        while (Load64(&b_completion_signal->sequence) < sequence &&
               Load64(&lifecycle->stop_requested) == 0) {
        ...
        ++report.output_fences;
        PublishDescriptor(
            host_result_descriptor, generation, request_id, elements, status, final_checksum, sequence
        );
        dsb(DSB_ALL);
