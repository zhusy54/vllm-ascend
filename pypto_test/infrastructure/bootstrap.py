...
class BootstrapManager:
    ...
    def __init__(
        ...
        *,
        ...
        memory_provider_factory: Callable[..., CrossDeviceMemoryProvider] = AscendVmmMemoryProvider,
    ) -> None:
        ...
        self._provider_factory = memory_provider_factory
        self._memory_provider: CrossDeviceMemoryProvider | None = None
    ...
    def build_communication(self) -> EndpointBundle:
        ...
        if self._memory_provider is None or self._lease is not None:
            raise BootstrapError("communication cannot build in the current state")

        # The RPC exchanges opaque manifests only.  Every provider imports the
        # peer handle locally, so no process-local VA crosses the Host boundary.
        local_manifest = self._memory_provider.allocate_shared_window()
        response = self._rpc.call("BUILD_COMMUNICATION", manifest=local_manifest.to_dict())
        peer_manifest = WindowManifest.from_dict(response["manifest"])
        self._memory_provider.attach_peer(peer_manifest)
        external_binding = self._memory_provider.binding(peer_manifest)
        npu_binding = _to_npu_binding(external_binding)
    ...
    def release(self) -> dict[str, Any]:
        ...
        if self._lease is None or self._memory_provider is None:
    ...
    def evidence(self) -> dict[str, Any]:
        return {
            ...
            "memory": self._memory_provider.evidence() if self._memory_provider is not None else None,
            "run_id": self.run_id,
        }
