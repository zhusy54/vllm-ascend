# PyPTO WSE 阶段 1B T04 验证记录

## 1. 结论

阶段 1B 的首个 case T04 已在两张 Ascend950PR 上完成 NPU surrogate 预验证：Attention
进程和 WSE surrogate 进程分别绑定 NPU 0/1，各自只启动一次 AIV kernel。两个 kernel
通过 CANN ACL VMM P2P 映射，在 Device Memory 中完成 100 次单 slot command/completion
闭环。
...
T04 只证明单 slot device loop 成立；完成 T05～T12 前不能声明达到 C2。真实 NPU-WSE
能力仍为 `NOT_ESTABLISHED`，`evidence_status` 仍为 `SIMULATION`。
...
最终功能代码 revision 为 `3a2582c5`。通过 `task-submit` 锁定 NPU 0/1，两种启动顺序均通过：
...
离线相关单元测试共 91 项通过，覆盖 Stage 0/1A 回归、AIV binary 生命周期、T04 契约、
结果聚合与证据脱敏。下一步按计划实现 T05 双 slot/流水，再验证 T06 背压和 T07 顺序可见性。
